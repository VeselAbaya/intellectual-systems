...
module.exports = {
    ...
    getPlayerPos(teamName) {
        ...
        const newFlags = coordsApi.getDistanceForOtherPlayer(player, flags)
        return coordsApi.getAnswerForThreeFlags(newFlags, FlagsCoords)
    },
    getVisible(goal, teamName) {
        // console.log(JSON.stringify(this.p.filter(obj => obj.cmd && obj.cmd.p[0] === 'p')))
        if (teamName) {
            return Boolean(this.p.find(obj => obj.cmd && obj.cmd.p[0] === goal && obj.cmd.p[1] === teamName))
        }
    ...
    getAngleToPass(teamName, playerPos) {
        const newPos = {
            x: playerPos[1].pos.x + 14*(playerPos[1].pos.x - playerPos[0].pos.x),
            y: playerPos[1].pos.y + 14*(playerPos[1].pos.y - playerPos[0].pos.y)
        }
        ...
        console.log(playerPos[1].angle, playerPos[0].angle, newAngle,
            (playerPos[1].angle - playerPos[0].angle > 0)
    ...
    getAngle(goal, teamName) {
        let goalObj
        if (teamName) {
            goalObj = this.p.find(obj => obj.cmd && obj.cmd.p[0] === goal && obj.cmd.p[1] === `"${teamName}"`)
            console.log(goalObj, teamName, this.p.find(obj => obj.cmd && obj.cmd.p[0] === goal))
