const flagDecisionTree = {
  ...
  rotate: {
    exec (mgr, state) { state.command = {n: "turn", v: 15} },
    next: "sendCommand",
  },
  rootNext: {
  ...
  runToGoal: {
    exec (mgr, state) { state.command = {n: "dash", v: 45} },
    next: "sendCommand",
  },
  sendCommand: {
  ...
  ballSeek: {
    condition: (mgr, state) => mgr.getDistance(state.action.flag) <= .5,
    trueCond: "closeBall",
    falseCond: "farGoal",
  },
  closeBall: {
    condition:
      (mgr, state) => mgr.getVisibleFlag(state.action.goal),
...
const twoPlayersDecisionTree = team => ({
  ...
  lookingDirectlyToTeammate: {
    condition: (mgr, state) => Math.abs(mgr.getVisiblePlayerFromTeam(team).direction) > 5,
  ...
  turnToTeammate: {
    exec: (mgr, state) => state.command = {n: "turn", v: mgr.getVisiblePlayerFromTeam(team).direction / 3},
    next: "sendCommand",
  },
  stayLeftToTeammate: {
    exec: (mgr, state) => state.command = {n: "turn", v: mgr.getVisiblePlayerFromTeam(team).direction - 15},
    next: "sendCommand",
  },
  dashToTeammate: {
  ...
  stayNearToTeammate: {
    exec: (mgr, state) => state.command = {n: "dash", v: mgr.getVisiblePlayerFromTeam(team).distance < 7 ? 20 : 40},
    next: "sendCommand",
  }
})
